...
import { Search as SearchIcon, Loader2, SearchX, Search as SearchLucide } from 'lucide-react'
...
export function Search() {
  ...
  const { query, setQuery, results, isLoading, error, search, clearSearch } = useSearch()
                ...
                >
                  {results.map(goal => (
                    <motion.button
                      ...
                      onClick={() => goal.goal_id && handleGoalClick(goal.goal_id)}
                    >
                      {goal.title}
                    </motion.button>
                  ))}
