...
export default function Goal({ goal, className = '' }: GoalProps) {
  ...
  return (
    <Link
      ...
      className={`flex flex-col sm:flex-row border hover:shadow-lg rounded-2xl hover:border-border-hover transition-shadow sm:h-[180px] ${className}`}
    >
      <div className='w-full h-full sm:w-1/2 sm:h-full'>
        <GoalImage goal={goal} className='h-full min-h-[178px]' />
      </div>
