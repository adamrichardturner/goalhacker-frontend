...
export default function SubGoals({ goal }: SubGoalsProps) {
  ...
  const [deletingSubgoalId, setDeletingSubgoalId] = useState<string | null>(null)
  ...
  const handleSubgoalStatusChange = (subgoalId: string, status: SubgoalStatus) => {
    // Optimistically update local state
    const previousSubgoals = [...localSubgoals]
    setLocalSubgoals(
      localSubgoals.map(sg => (sg.subgoal_id === subgoalId ? { ...sg, status } : sg))
    )
  ...
  const handleEditSave = () => {
    ...
    setLocalSubgoals(
      localSubgoals.map(sg =>
        sg.subgoal_id === editing.subgoalId ? { ...sg, title: editing.title } : sg
      )
    )
  ...
  const handleDeleteConfirm = () => {
    ...
    const previousSubgoals = [...localSubgoals]
    setLocalSubgoals(localSubgoals.filter(sg => sg.subgoal_id !== deletingSubgoalId))
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="space-y-4 py-4">
              ...
              >
                <SelectTrigger className="w-full sm:w-[140px] h-12">
                  <SelectValue placeholder="Status" className="text-muted-foreground" />
                </SelectTrigger>
                <SelectContent>
            ...
            <Popover>
              <PopoverTrigger asChild className="w-12 h-12 bg-input">
                <Button
                  ...
                  className={cn(
                    'h-12 w-12 bg-input',
                    newSubgoal.target_date && 'border-2 bg-input border-electricPurple'
                  )}
                >
      ...
      <Reorder.Group
        ...
        className={cn('space-y-3', isDraggable && !isEditing() && 'cursor-grab')}
      >
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
              <Select
                ...
                onValueChange={(value: SubgoalStatus) =>
                  subgoal.subgoal_id && handleSubgoalStatusChange(subgoal.subgoal_id, value)
                }
              >
                <SelectTrigger className="w-full sm:w-[140px] h-12">
                  <SelectValue placeholder="Status" className="text-muted-foreground" />
              ...
              {editing.subgoalId !== subgoal.subgoal_id && (
                <>
                  ...
                  <Button
                    ...
                    onClick={() => subgoal.subgoal_id && handleDeleteClick(subgoal.subgoal_id)}
                  >
      ...
      <AlertDialog open={!!deletingSubgoalId} onOpenChange={() => setDeletingSubgoalId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeletingSubgoalId(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction
